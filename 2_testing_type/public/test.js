$(document).ready(function() {
	$.mockjax({
		url: "/api",
		status: 200,
		responseText: 'hello from test.js'
	});
	// $.mockjax({
	// 	url: "/api",
	// 	status: 400,
	// 	responseText: 'error from test.js'
	// });
});
...
(function(root, factory) {
	...
	function getMockForRequest(handler, requestSettings) {
		...
		if ($.isFunction(handler.url.test)) {
			...
			var namespace = handler.namespace || (typeof(handler.namespace) === 'undefined' && $.mockjaxSettings
				.namespace);
			...
			if (effecitveUrl !== requestSettings.url && star === -1 ||
				!new RegExp(effecitveUrl.replace(/[-[\]{}()+?.,\\^$|#\s]/g, '\\$&').replace(/\*/g, '.+')).test(
					requestSettings.url)) {
				return null;
			}
		}
	...
	function _xhrSend(mockHandler, requestSettings, origSettings) {
		logger.debug(mockHandler, ['Sending fake XHR request', mockHandler, requestSettings,
			origSettings
		]);
			...
			return function() {
				return (function() {
					...
					var finishRequest = function() {
						...
						} else if (typeof mockHandler.status === 'number' || typeof mockHandler.status ===
							'string') {
							this.status = mockHandler.status;
						}
	...
	function xhr(mockHandler, requestSettings, origSettings, origHandler) {
		logger.debug(mockHandler, ['Creating new mock XHR object', mockHandler, requestSettings,
	...
	function processJsonpMock(requestSettings, mockHandler, origSettings) {
		...
		if (requestSettings.data && CALLBACK_REGEX.test(requestSettings.data) || CALLBACK_REGEX.test(
				requestSettings.url)) {
	...
	function processJsonpRequest(requestSettings, mockHandler, origSettings) {
		logger.debug(mockHandler, ['Performing JSONP request', mockHandler, requestSettings,
			origSettings
		]);
	...
	function createJsonpCallback(requestSettings, mockHandler, origSettings) {
		var callbackContext = origSettings && origSettings.context || requestSettings;
		var jsonp = (typeof requestSettings.jsonpCallback === 'string' && requestSettings.jsonpCallback) ||
			('jsonp' + jsc++);
	...
	function handleAjax(url, origSettings) {
		...
		for (var k = 0; k < mockHandlers.length; k++) {
			var handlerIndex = $.mockjaxSettings.matchInRegistrationOrder ? k : mockHandlers.length - 1 -
				k;
			...
			if ((mockHandler.status === 301 || mockHandler.status === 302) &&
				(requestSettings.type.toUpperCase() === 'GET' || requestSettings.type.toUpperCase() ===
					'HEAD') &&
	...
	var logger = {
		_log: function logger(mockHandler, args, level) {
			...
			} else if ($.mockjaxSettings.logger && $.mockjaxSettings.logger[$.mockjaxSettings.logLevelMethods[
					level]]) {
				return $.mockjaxSettings.logger[$.mockjaxSettings.logLevelMethods[level]].apply($.mockjaxSettings
					.logger, args);
			}
		},
		/**
